/* ==========================================================================
   STAR WARS RPG - ENGINE DE RENDERIZAÇÃO DA HIERARQUIA IMPERIAL
   ========================================================================== */
...
const dadosImperio = {
    politico_imp: {
        ...
        patentes: [
            ...
            { nome: "Moff", desc: "Governador de um sector imperial. Administra a política local e atua como líder militar máximo dentro das fronteiras de seu setor.", blocos: [["B", "B", "B", "B", "B", "B"], ["R", "R", "R", "Y", "Y", "Y"]], cilindros: 2 }
        ]
    },
    marinha_imp: {
...
function gerarPlacaImperialSVG(matriz, numCilindros, ombreira) {
    ...
    // Paleta de cores oficial do Império (Incluindo o Amarelo 'Y')
    ...
    if (maxColunas === 0 && matriz[0]) {
        maxColunas = matriz[0].length;
    }

    // Geração de IDs dinâmicos únicos para evitar conflitos de renderização entre os cards
    const idUnicoGradiente = "metal-imp-" + Math.floor(Math.random() * 1000000);
    const idUnicoGlow = "glow-imp-" + Math.floor(Math.random() * 1000000);
    
    // Ajuste dimensional proporcional da placa de cromo
    ...
    matriz.forEach((linha, idxLinha) => {
        linha.forEach((tipo, idxColuna) => {
            if (tipo && cores[tipo]) {
                ...
                blocosHTML += `
                    <!-- Efeito de iluminação interna sutil -->
                    <rect x="${x}" y="${y}" width="${blocoLargura}" height="${blocoAltura}" fill="${cores[tipo]}" filter="url(#${idUnicoGlow})" opacity="0.35"/>
                    <!-- Bloco Sólido Quadrado/Retangular -->
                    <rect x="${x}" y="${y}" width="${blocoLargura}" height="${blocoAltura}" fill="${cores[tipo]}" stroke="#000" stroke-width="1.5" rx="1"/>
                    <!-- Brilho angular para simular acrílico/cristal -->
    ...
    // Construção dos Cilindros de Código (Code Cylinders) metálicos na lateral direita
    let cilindrosHTML = '';
    ...
    for (let i = 0; i < numCilindros; i++) {
        ...
        cilindrosHTML += `
            ...
            <!-- Corpo do núcleo cilíndrico metálico -->
            <rect x="${xCilindro}" y="${yTopCilindro}" width="12" height="44" rx="2" fill="url(#${idUnicoGradiente})" stroke="#222" stroke-width="1"/>
            <!-- Cabeça prismática de dados -->
            <rect x="${xCilindro + 2}" y="${yTopCilindro}" width="8" height="6" fill="#00aeff" stroke="#111" stroke-width="0.5"/>
            <!-- Presilha de cromo do bolso -->
    ...
    return `
        <svg width="${larguraTotalSVG}" height="${alturaTotalSVG}" viewBox="0 0 ${larguraTotalSVG} ${alturaTotalSVG}">
            <defs>
                <filter id="${idUnicoGlow}" x="-20%" y="-20%" width="140%" height="140%">
                    <feGaussianBlur stdDeviation="3" result="blur" />
                </filter>
                <linearGradient id="${idUnicoGradiente}" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <!-- Placa Base de Liga Metálica Escura -->
            <rect x="4" y="6" width="${larguraInsignia - 8}" height="${alturaInsignia}" rx="2" fill="url(#${idUnicoGradiente})" stroke="#111" stroke-width="2"/>
...
// Inicialização da Árvore de Dados Imperial
