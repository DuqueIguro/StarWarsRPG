async function verificarCredenciaisISB() {
    ...
    loadingDiv.innerHTML = `
        <div style="position:fixed; top:0; left:0; width:100%; height:100%; background:#050308; z-index:99999; display:flex; flex-direction:column; align-items:center; justify-content:center; color:#ef4444; font-family:monospace; text-align:center;">
            ...
            <h2 style="font-size:1.5rem; font-weight:bold; letter-spacing:0.2em;">VERIFICANDO CREDENCIAIS...</h2>
    ...
    if (permError) {
        console.error("[GATEKEEPER] Erro no banco de dados:", permError);
    }

    if (permError || !perm || (perm.cargo !== 'mestre' && perm.cargo !== 'dev' && perm.cargo !== 'durtoc')) {
        alert("ACESSO NEGADO: Suas credenciais não possuem autorização. O incidente foi registrado.");
